...
def forward_backwardOld(score, noiseScore):
    ...
    grad = grad - 2 * torch.diag_embed(
        F.softplus(torch.diagonal(score, dim1=0, dim2=1)), dim1=0, dim2=1
    )
...
def evalPath(intervals: List[List[Tuple[int, int]]], score, noiseScore):
    ...
    indices_neg = [
        idx + i * nBatch for idx, curList in enumerate(intervals) for i, j in curList
    ]
    indices_pos = [
        idx + j * nBatch for idx, curList in enumerate(intervals) for i, j in curList
    ]
