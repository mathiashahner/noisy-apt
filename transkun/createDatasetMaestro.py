...
if __name__ == "__main__":
    ...
    argumentParser.add_argument(
        "datasetPath", help="folder path to the maestro dataset"
    )
    argumentParser.add_argument(
        "metadataCSVPath", help="path to the metadata file of the maestro dataset (csv)"
    )
    argumentParser.add_argument("outputPath", help="path to the output folder")
    argumentParser.add_argument(
        "--noPedalExtension",
        action="store_true",
        help="Do not perform pedal extension according to the sustain pedal",
    )
    ...
    dataset = Data.createDatasetMaestroCSV(
        datasetPath, datasetMetaCSVPath, extendSustainPedal=extendPedal
    )
