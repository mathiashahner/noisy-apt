...
class TransKun(torch.nn.Module):
    ...
    def __init__(self, conf):
        ...
        self.framewiseFeatureExtractor = MelSpectrum(
            conf.windowSize,
            f_min=conf.f_min,
            f_max=conf.f_max,
            n_mels=conf.n_mels,
        ...
        if useInnerProductScorer:
            ...
            self.scorer = PairwiseFeatureBatch(
                512, outputSize=len(self.targetMIDIPitch)
            )
    ...
    def processFramesBatch(self, framesBatch):
        ...
        ctx = self.backbone(
            featuresBatch,
            outputIndices=torch.tensor(
                self.targetMIDIPitch, device=featuresBatch.device
            ),
        )
    ...
    def log_prob(self, xBatch, notesBatch):
        ...
        if nIntervalsAll > 0:
            ctx_a_all, ctx_b_all, symIdx_all, scatterIdx_all = (
