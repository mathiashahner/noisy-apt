...
def eval(args):
    ...
    notesGT = Data.parseMIDIFile(
        str(targetPath),
        extendSustainPedal=extendSustainPedal,
        pedal_ext_offset=pedalOffset,
    )
    ...
    # recompute
    metrics = Evaluation.compareTranscription(
        notesEst, notesGT, splitPedal=True, computeDeviations=computeDeviations
    )
...
def main():
    ...
    argParser = argparse.ArgumentParser(
        description="compute metrics directly from MIDI files."
        + os.linesep
    ...
    argParser.add_argument(
        "--outputJSON",
        help="path to save the output file for detailed metrics per audio file",
    )
    argParser.add_argument(
        "--noPedalExtension",
        action="store_true",
        help="Do not perform pedal extension according to the sustain pedal for the ground truch",
    )
    argParser.add_argument(
        "--applyPedalExtensionOnEstimated",
        action="store_true",
        help="perform pedal extension for the estimated midi",
    )
    argParser.add_argument(
        "--nProcess",
        nargs="?",
    ...
    argParser.add_argument(
        "--alignOnset", action="store_true", help="whether or not realign the onset."
    )
    argParser.add_argument(
        "--dither",
        default=0.0,
        type=float,
        help="amount of noise added to the prediction.",
    )
    argParser.add_argument(
        "--pedalOffset",
        default=0.0,
        type=float,
        help="offset added to the groundTruth sustain pedal when extending notes",
    )
    ...
    filenames = list(estPath.glob(os.path.join("**", "*.midi"))) + list(
        estPath.glob(os.path.join("**", "*.mid"))
    )
