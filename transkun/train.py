...
def train(workerId, nWorker, filename, runSeed, args):
    ...
    (
        ...
        optimizer,
        lrScheduler,
    ) = load_checkpoint(TransKun, conf, filename, device, args=args)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--nProcess",
        help="# of processes for parallel training",
        required=True,
        type=int,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--master_port",
        help="master port number for distributed training",
        default="29500",
    )
    ...
    if num_processes == 1:
        ...
        mp.spawn(
            fn=train,
            args=(num_processes, saved_filename, runSeed, args),
