    ...
    if restartFromTheBest:
        if not strict:
            load_state_dict_tolerant(
                model,
                get_checkpoint_property(checkpoint, "best_state_dict", best_state_dict),
            )
        else:
        ...
        if not strict:
            load_state_dict_tolerant(
                model, get_checkpoint_property(checkpoint, "state_dict")
            )
        else:
            model.load_state_dict(get_checkpoint_property(checkpoint, "state_dict"))

        optimizer.load_state_dict(
            get_checkpoint_property(
        ...
        lrScheduler.load_state_dict(
            get_checkpoint_property(
                checkpoint, "lr_scheduler_state_dict", lrScheduler.state_dict()
            )
        )

    # lrScheduler.total_steps = 500000
    # print(optimizer.param_groups)
    best_state_dict = get_checkpoint_property(
