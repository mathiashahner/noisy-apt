...
class ConvBlock_ablation(nn.Module):
    def __init__(
        self, inputSize, outputSize, hiddenSize, kernelSize, stride=1, dropoutProb=0.0
    ):
        super().__init__()
        ...
        self.conv1 = nn.Conv2d(
            inputSize, hiddenSize, kernelSize, padding=kernelSize // 2
        )

        self.bn1 = BatchNorm2d(hiddenSize, momentum=0.01)

        self.conv2 = nn.Conv2d(
            hiddenSize, outputSize, kernelSize, padding=kernelSize // 2
        )
...
class PairwiseFeatureBatch(nn.Module):
    def __init__(
        ...
        if postConv:
            self.post = ScoreMatrixPostProcessor(
                outputSize, outputSize * 3, dropoutProb
            )
    ...
    def computeChunk(self, x, x_cum, x_sqr_cum, x_cube_cum, idxA, idxB):
        ...
        curInput = torch.cat(
            [curA, curB, curA * curB, moment1, moment2, moment3], dim=-1
        )
    ...
    def forward(self, x, nBlock=4000):
        ...
        for lIdx in range(0, nTotal, nBlock):
            ...
            curScore = checkpoint(
                self.computeChunk, x, x_cum, x_sqr_cum, x_cube_cum, idxA, idxB
            )
        ...
        S_coo = torch.sparse_coo_tensor(
            indices, s_val, (nEntry, nEntry, s_val.shape[-2], s_val.shape[-1])
        )
