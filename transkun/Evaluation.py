...
def compareTranscription(
    estimated, gt, splitPedal=False, computeDeviations=False, **kwargs
):
    ...
    metrics["frame"] = computeFrameScore(
        estimated, gt, eventTypes=list(range(21, 108 + 1))
    )
    ...
    if len(pedalEst) > 0:
        ...
        for cc in pedalEst:
            ...
            if nGTPedal > 0:
                metrics["pedal" + str(cc) + "frame"] = computeFrameScore(
                    estimated, gt, eventTypes=[-cc]
                )
                metrics["pedal" + str(cc)] = (
