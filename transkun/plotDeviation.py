...
parser = argparse.ArgumentParser(
    description="plot the empirical cumulative distribution function on onset/offset deviations"
)

parser.add_argument(
    "evalJsons",
    nargs="+",
    help="a seqeunce of the output json files from the computeMetrics script, the deviation output should be enabled",
)
...
parser.add_argument(
    "--offset",
    action="store_true",
    help="plot the offset deviation curve. If not specified, onset deviation curve will be plotted",
)
...
parser.add_argument(
    "--cumulative", action="store_true", help="plot the empirical cumulative density."
)
parser.add_argument("--absolute", action="store_true", help="use absolute deviation.")
parser.add_argument(
    "--targetPitch", required=False, type=int, help="only plot specific number."
)
